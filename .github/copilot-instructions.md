...
- Data flow: HuggingFace dataset (full download, no streaming) → ETL filter in src/data_loader.py → materialized pandas DataFrame (saved under data/) → preprocessing → modeling (linear/nonlinear/chaos).
...
- Dataset: Dingdong-Inc/FreshRetailNet-50K (Parquet, ~108MB). Use load_dataset(..., streaming=False) and load the full train split into memory.
...
- Unit tests must mock external calls (especially HuggingFace dataset download). Integration tests should be marked with @pytest.mark.integration.
